...
public interface TpmOpinionRankDao  extends JpaRepository<OpinionRank,Long>{
    @Query(value = "select o from OpinionRank o where o.year=?1 and o.month=?2 and o.productType=?3 and o.modifyId is null order by o.rank ")
    List<OpinionRank> getWithOutDataType(Integer year,Integer month,Integer productType);

    @Query(value = "select o from OpinionRank o where o.year=?1 and o.month=?2 and o.productType=?3 and o.dataType=?4 and o.modifyId is null order by o.rank ")
