...
public interface TpmHotWordDataDao extends JpaRepository<TpmProductFeeling,Long> {
    @Query(nativeQuery = true,value = "select * from tpm_hotword_data where deleted=0 and modify_id is null and deleted=0 and year=?1 and month=?2 and product_type=?3")
